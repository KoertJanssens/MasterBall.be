...
def search_overseer_thread(args, new_location_queue, pause_bit, heartb, db_updates_queue, wh_queue):
    ...
    while True:
        ...
        if not new_location_queue.empty():
            ...
            step_distance = 0.500 if args.no_pokemon else 0.070
...
def token_request(args, status, url, whq):
    ...
    if args.captcha_key is None:
        ...
        if args.webhooks:
            ...
            if token_needed>=5 and token_needed>=0.8*args.workers:
                whq.put(('captcha', {'account': 'MasterBall', 'status': 'kurt', 'token_needed': token_needed}))
...
def stagger_thread(args, account):
    ...
    delay = 1 + args.login_delay * random.random()
